...
- \`submission/\`：当前正式的两个选手文件；
- \`packages/\`：完整包、简略包和两包合集；
- \`docs/\`：英文运行和交付说明；
- \`README.md\`：本中英双语说明。
...
\`IDC2026_Team_Orange_Cyber_Game_Submission_Complete.zip\` 包含 0814 官方运行环境、
场景、MuJoCo 模型、资源文件、当前 Team Orange 参赛文件和 PowerShell 启动脚本。
它适合需要完整运行包或本地复现的场景，但电脑仍需安装 Python 和第三方依赖。

#### 简略包

\`IDC2026_Team_Orange_Cyber_Game_Submission_Minimal.zip\` 只包含两个选手编写的文件和
说明文档。若主办方要求只提交选手文件，应把其中两个文件复制到官方 Cyber Game
运行目录。

#### 合集包

\`IDC2026_Team_Orange_Cyber_Game_Submission_Bundle.zip\` 是外层交付包，包含完整包、
简略包以及介绍两个压缩包区别的英文说明。
